# Sprint 26-13

## Genia Mobile App [project:genia mobile app] [release:mthm1jxa_3h9dem]
- [x] Cacheamento endpoints (Gui + Vicco)
- [x] Dashboard (Hary) [HIGH]
- [x] Catálogo Neodent- pacote pronto para ser integrado (Anderson) [HIGH]
- [x] Filtros avançados (Nu)
- [x] Jornada do cliente produtos digital (Gui + Nu) [BLOCKED]
- [x] Analise biometria SSO - Sync (Andy) [HIGH]
- [x] Campanhas mensais (Andy + Nu)
- [x] Multimarca (Andy + Nu)
- [x] Quick Win 4 + 2 (Andy + Nu)
- [x] Authentication SSO (Andy + Nu)
- [x] Alteração Campos de Crédito (Andy + Nu)

## Genia Web App [project:genia web app] [release:mthm1jxa_upc1wb]
- [x] Testes multimarca + sso + campanha mensais (Nu + Bruf) [HIGH]
- [x] Hierarquia dev (Bruf) [BLOCKED]
- [x] Markdown Genia (Brunin + Bruf)
- [x] WakeWord (Brunin + Sam) [BLOCKED]
- [x] Echo Cancelation (Brunin + Sam) [BLOCKED]

## Outros [project:outros] [release:mthm1jxa_ovlo7f]
- [x] SAP -> Acompanhamento Bloqueio ZSEB -> Teste falhou, aguardando retorno (Fel) [BLOCKED]
- [x] SAP -> Acompanhamento Criacao de Cotacao -> Preciso de att (Fel) [BLOCKED]
- [x] SAP -> Acompanhamento Erro NFs nos pedidos -> Acompanhamento (Gui) [BLOCKED]
- [x] SAP -> Acompanhamento adicao de ZSEB / Clear / Consignado no pedido -> Aguardando Aprovacao do Global (Fel) [BLOCKED]
- [x] SAP -> Acompanhamento cotação ClearWallet -> Acompanhamento (Gui) [BLOCKED]
- [x] Aguardando novo contrato neodent app (Nu + Aline compliance) [BLOCKED]
- [x] Endpoints CRM de visita e confirmação + adição de comentários
- [x] Lambda Pipeline (Brunin + Bruf + Gui)
- [x] Migração Alteryx (Fel) [HIGH]
- [x] Alinhamento problemas ONEID (Nu)

## Mapping [project:mapping] [release:mthm1jxa_d8dfdt]
tão enrolando

- [x] Aguardando IPs autorizados (Guto) [BLOCKED]
- [x] Teste (Guto + Nu) [HIGH] [BLOCKED]
- [x] Espelhamento da base prod (Guto) [BLOCKED]

## OTB [project:otb] [release:mthm1jxa_kpa31w]
- [x] Planejamento de correção definitiva de autenticação - KID (Gui) [BLOCKED]
- [x] Alinhamento novo Fluxo? (Fel)

## Hub [project:global hub] [release:mthm1jxa_fmes0n]
- [x] Teste categorias FAQ (Nu)
- [x] Tela Kit resgate admin (Berg)
- [x] acompanhamento FAQ (Maxo)
- [x] Back Kit resgate (Guto)

## Straumann Mobile App [project:straumann mobile app] [release:mthm1jxa_90scyo]
- [x] Refinamento tela de reconhecimento (Andy + Nu) [HIGH] [BLOCKED]
- [x] Trade Up (?) (Fel) [BLOCKED]
- [x] Straumann Web (Rich) [BLOCKED]
- [x] Dev Mobile Who is Who (Vicco)
- [x] Dev Back Who is Who (Gui) [BLOCKED]

## Kit Resgate [project:kit recuperacao] [release:mthm1jxa_b2w22b]
- [x] Aprovação tela do hub admin (Nu)
- [x] Arquitetura BD (Guto)
- [x] Desenvolvimento mobile (Dani)
- [x] Desenvolvimento Front (Berg)

## Eventos [project:eventos] [release:mthm1jxa_bf4sa2]
- [ ] Aguardando decisao do LT

## Neodent Mobile App [project:neodent mobile app] [release:mthm1jxa_fk4wjm]
- [x] Telas Rastreio 2.0 (Maxo + Vicco)
- [x] Link do rastreio AfterShip (Bruf + Maxo + Vicco)
- [x] Sessão Convidado (Dani + Nu) [BLOCKED]
- [x] Teste OIDC para permitir redirect login straumann (Nu)

## ClearWallet [project:clearwallet] [release:mthm1jxa_wr2l3g]
- [x] Conexão direta Dr.Portal (Fel) [HIGH]
- [x] Definições atividades Sync (Fel) [HIGH]
- [x] Recuperar ambiente de homolog (Bruf + Gui)

## ClearCorrect [project:clearcorrect automations] [release:mthm1jxa_9jke9n]
- [x] Formulário de Reclamações (Nu) [BLOCKED]
- [x] Máquina para automação (Fel) [HIGH]
