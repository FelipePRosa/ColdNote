# Sprint 26-12

## Genia Mobile App [project:genia mobile app] [release:mthm1jx9_macciv]
- [x] Dashboard (Brunin) [HIGH]
- [x] Jornada do cliente produtos digital (Gui + Nu) [BLOCKED]
- [x] KPIs tela + logicas (Gui + Nu + Fel) [HIGH]
- [x] Analise biometria SSO (Andy) [HIGH]
- [x] Multimarca (Andy)
- [x] Quick Win 4 + 2 (Gui + Andy)
- [x] Authentication SSO (Gui + Vicco)

## Genia Web App [project:genia web app] [release:mthm1jx9_djfhah]
- [x] Testes multimarca + sso + campanha mensais (Nu + Bruf) [HIGH]
- [x] Hierarquia dev (Bruf) [BLOCKED]
- [x] Markdown Genia (Brunin + Bruf)
- [x] Quick Win 4 + 2 (Gui + Bruf)

## Outros [project:outros] [release:mthm1jx9_i1d4bh]
- [x] SAP -> Acompanhamento Bloqueio ZSEB -> Teste falhou, aguardando retorno (Fel) [BLOCKED]
- [x] SAP -> Acompanhamento Criacao de Cotacao -> Preciso de att (Fel) [BLOCKED]
- [x] SAP -> Acompanhamento Erro NFs nos pedidos -> Acompanhamento (Gui) [BLOCKED]
- [x] SAP -> Acompanhamento adicao de ZSEB / Clear / Consignado no pedido -> Aguardando Aprovacao do Global (Fel) [BLOCKED]
- [x] Divulgacao Straumann + Sales App (Mkt)
- [x] Aguardando novo contrato neodent app (Nu + Aline compliance) [BLOCKED]

## Mapping [project:mapping] [release:mthm1jx9_t8kak8]
tão enrolando

- [x] Aguardando IPs autorizados (Guto) [BLOCKED]
- [x] Teste (Guto + Nu) [HIGH]
- [x] Espelhamento da base prod (Guto) [BLOCKED]

## OTB [project:otb] [release:mthm1jx9_w0cbyv]
- [x] Planejamento de correcao definitiva de autenticacao - KID (Gui) [BLOCKED]

## Hub [project:global hub] [release:mthm1jx9_j3m0ju]
- [x] Teste categorias FAQ (Nu)
- [x] Acompanhamento FAQ (Maxo)
- [x] Back Kit resgate (Guto)

## Straumann Mobile App [project:straumann mobile app] [release:mthm1jxa_6s7drf]
- [x] Refinamento tela de reconhecimento (Andy + Nu) [HIGH] [BLOCKED]
- [x] Trade Up (?) (Fel) [BLOCKED]
- [x] Straumann Web (Rich) [BLOCKED]
- [x] Dev Mobile Who is Who (Vicco)
- [x] Dev Back Who is Who (Gui) [BLOCKED]

## Kit Resgate [project:kit recuperacao] [release:mthm1jxa_duabil]
- [x] Aprovação tela do hub admin (Nu)
- [x] Arquitetura BD (Guto)
- [x] Desenvolvimento mobile (Dani)
- [x] Desenvolvimento Front (Berg)

## Eventos [project:eventos] [release:mthm1jxa_3fag8s]
- [x] Aguardando decisao do LT (Fel) [BLOCKED]

## Neodent Mobile App [project:neodent mobile app] [release:mthm1jxa_ir8n7g]
- [x] Dev Rastreio 2.0 (Maxo)
- [x] Telas Rastreio 2.0 (Nu + Rich)
- [x] Sessão Convidado (Dani + Nu) [BLOCKED]
- [x] Ignorar chamada de endpoints app desatualizado (problema Fortress) (Vicco)
- [x] Refatorando OIDC para permitir redirect login straumann (Vicco)
- [x] Deploy catalogo 2026 (Vicco)

## ClearWallet [project:clearwallet] [release:mthm1jxa_1l8m8n]
- [x] Conexão direta Dr.Portal (Gui) [HIGH]
- [x] Definições atividades Sync (Fel) [HIGH]
