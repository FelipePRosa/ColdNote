# Sprint 26-11

## Genia Mobile App [project:genia mobile app] [release:mthm1jx9_aq2x3c]
- [x] Cacheamento endpoints (Gui + Vicco)
- [x] Dashboard (Brunin) [HIGH]
- [x] Catálogo Neodent (Vicco) [HIGH]
- [x] Filtros avançados (Gui + Nu)
- [x] Jornada do cliente produtos digital (Gui + Nu) [BLOCKED]
- [x] KPIs tela + logicas (Gui + Nu) [HIGH]
- [x] Desenvolvimento SSO (Andy) [HIGH]
- [x] Multimarca (Andy)

## Genia Web App [project:genia web app] [release:mthm1jx9_9s8sop]
- [x] Testes (Nu + Berg) [HIGH]
- [x] Hierarquia dev (Berg) [BLOCKED]
- [x] Desenvolvimento SSO (Bruf)
- [x] Desenvolvimento Multimarca (Bruf)
- [x] Promoções ativas (Bruf)

## Outros [project:outros] [release:mthm1jx9_xmdbz0]
- [x] SAP -> Acompanhamento Bloqueio ZSEB -> Teste falhou, aguardando retorno (Fel) [BLOCKED]
- [x] SAP -> Acompanhamento Criacao de Cotacao -> Preciso de att (Fel) [BLOCKED]
- [x] SAP -> Acompanhamento Erro NFs nos pedidos -> Acompanhamento (Gui) [BLOCKED]
- [x] SAP -> Acompanhamento adicao de ZSEB / Clear / Consignado no pedido -> Aguardando Aprovacao do Global (Fel) [BLOCKED]
- [x] Divulgacao Straumann + Sales App (Mkt)
- [x] Aguardando novo contrato neodent app (Nu) [BLOCKED]

## Mapping [project:mapping] [release:mthm1jx9_aqoasj]
- [x] Aguardando IPs autorizados (Guto) [BLOCKED]
- [x] Teste (Guto + Nu) [HIGH]
- [x] Espelhamento da base prod (Guto) [BLOCKED]

## OTB [project:otb] [release:mthm1jx9_ununow]
- [x] Planejamento de correcao definitiva de autenticacao - KID (Gui) [BLOCKED]

## Hub [project:global hub] [release:mthm1jx9_sx4iqy]
- [x] Teste categorias FAQ (Nu)

## Straumann Mobile App [project:straumann mobile app] [release:mthm1jx9_n94doy]
- [x] Refinamento tela de reconhecimento (Andy) [HIGH]
- [x] Trade Up (?) (Fel) [BLOCKED]
- [x] Straumann Web (Rich) [BLOCKED]
- [x] Dev Mobile Who is Who (Andy)
- [x] Dev Back Who is Who (Gui)

## Kit Recuperacao [project:kit recuperacao] [release:mthm1jx9_c1c9ep]
- [x] Revisando templates (Nu)
- [x] Aprovação de formularios (Nu)
- [x] Arquitetura BD (Guto)
- [x] Desenvolvimento mobile (Dani)
- [x] Desenvolvimento Front (Berg)

## Eventos [project:eventos] [release:mthm1jx9_nsii7g]
- [x] Aguardando decisao do LT (Fel) [BLOCKED]

## Neodent Mobile App [project:neodent mobile app] [release:mthm1jx9_gpk00q]
- [x] Atualizacao catalogo (Dani) [HIGH]
- [x] Alteração SSO (aguardando Bruf) (Brunin + Gui + Vicco)
- [x] Busca de catalogo melhorada (Guto + Brunin + Vicco) [HIGH]
- [x] Novos SKUs (Brunin + Nu) [HIGH]
- [x] Refatoração script catálogo (Brunin) [HIGH]
- [x] Validação Catalogo (Nu)
- [x] Rastreio 2.0 (Guto)
- [x] Telas Rastreio 2.0 (Nu + Rich)
- [x] Sessão Convidado (Dani + Nu) [BLOCKED]
- [x] Ajeitar ordem de endpoints (problema Fortress) (Vicco)

## ClearWallet [project:clearwallet] [release:mthm1jx9_weseyo]
- [x] Conexão direta Dr.Portal (Gui) [HIGH]
- [x] 2 quick wins (reativar link + expiração 5 dias) (Gui + Berg) [HIGH]
- [x] Definições atividades Sync (Fel) [HIGH]
- [x] Revalidar criptografia pagamento (Gui + Nu)
